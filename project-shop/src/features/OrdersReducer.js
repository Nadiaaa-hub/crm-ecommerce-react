...
//   return (JSON.parse(localStorage.getItem("deletedOrders") || "[]")).map(id => String(id));
// }
...
//     const result = await axios.delete(`${ORDERS_URL}/${orderId}`);
//     return orderId;
    ...
    reducers: {
        setSearch(state, action) {
            ...
            state.filtered = state.data.filter(order => {
                const orderId = order.id.toLowerCase().includes(search);
        ...
        //     state.filtered = state.filtered.filter(order => order.id !== id);
            
        //     const deleted = JSON.parse(localStorage.getItem("deletedOrders") || "[]").map(String);
