...
export default function RecentClients({ limit = 4 }) {
  ...
  return (
    <div className="recentClients">
      ...
      {error ? (
        <div className="dash__empty">{error}</div>
      ) : recent.length === 0 ? (
        <div className="dash__empty">No clients yet</div>
      ) : (
        <div className="recentClients__list">
          {recent.map((c) => {
            ...
            const extra =
              c.email ||
              c.phone ||
              (Array.isArray(c.orders) ? `Orders: ${c.orders.length}` : "");
            ...
            return (
              <div className="clientRow" key={c.id ?? name}>
                ...
                <div className="clientRow__right">
                  <span className="clientRow__badge">new</span>
                </div>
              </div>
            );
          })}
